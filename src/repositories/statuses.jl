...
function create_status(repo, sha; options...)
    result = gh_post_json("/api/v3/projects/$(repo.id))/statuses/$(name(sha)"; options...)
    return Status(result)
end

function statuses(repo, ref; options...)
    results, page_data = gh_get_paged_json("/api/v3/projects/$(repo.id))/repository/commits/$(name(ref)/statuses"; options...)
    return map(Status, results), page_data
end

#= TODO: no equivalent API
function status(repo, ref; options...)
    result = gh_get_json("/api/v3/projects/$(repo.id))/commits/$(name(ref)/status"; options...)
    return Status(result)
end
=#
