...
function commits(repo; options...)
    ...
    results, page_data = gh_get_paged_json("/api/v3/projects/$(repo.id)/repository/commits"; options...)
...
function commit(repo, sha; options...)
    ...
    result = gh_get_json("/api/v3/projects/$(repo.id))/repository/commits/$(name(sha))"; options...)
