...
function stargazers(repo; options...)
    ...
    results, page_data = gh_get_paged_json("/api/v3/projects/$(repo.id)/star"; options...)
...
star(repo; options...) = gh_post("/api/v3/projects/$(repo.id)/star"; options...)

## unstar(repo; options...) = gh_delete("/user/starred/$(name(repo))"; options...)
unstar(repo; options...) = gh_delete("/api/v3/projects/$(repo.id)/star"; options...)
...
function watchers(repo; options...)
    ...
    results, page_data = gh_get_paged_json("/api/v3/projects/$(repo.id)/subscribers"; options...)
    return map(Owner, results), page_data
end
...
watch(repo; options...) = gh_put("/api/v3/projects/$(repo.id)/subscription"; options...)

## unwatch(repo; options...) = gh_delete("/repos/$(name(repo))/subscription"; options...)
unwatch(repo; options...) = gh_delete("/api/v3/projects/$(repo.id)/subscription"; options...)
=#
